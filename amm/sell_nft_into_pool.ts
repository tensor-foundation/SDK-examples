...
export async function sellNftIntoPool(mint: string, poolAddress: string) {
  ...
  if (poolType === PoolType.NFT)
    throw new Error(
      `Pool ${poolAddress} is a sell-side only pool and is not bidding.`
    );
  ...
  if (cosigner)
    throw new Error(
      `Pool ${poolAddress} has a cosigner and requires a cosignature. Remove this line if you are in possession of the cosigners private key! :)`
    );
  ...
  const {
    tokenStandard,
    data: { creators: creatorsRaw },
    programmableConfig,
  } = await fetchMetadata(rpc, metadataPda);
  ...
  if (!minPrice)
    throw new Error(`Pool ${poolAddress} does not have enough funds left.`);
  const sellNftAsyncInput:
    | SellNftTokenPoolAsyncInput
    | SellNftTradePoolAsyncInput = {
    owner: owner,
    whitelist: whitelist,
  ...
  if (poolType === PoolType.Token)
    sellIx = await getSellNftTokenPoolInstructionAsync(sellNftAsyncInput);
  else if (poolType === PoolType.Trade)
    sellIx = await getSellNftTradePoolInstructionAsync(sellNftAsyncInput);
