...
async function buyLegacy(mint: string, whitelist: string) {
  ...
  if (
    ...
    (isSome(privateTaker) &&
      unwrapOption(privateTaker) != keypairSigner.address) ||
    ...
    throw new Error(
      "This listing can't be bought via SDK. It either specified a differing private taker or needs a valid cosignature!"
    );
  }
